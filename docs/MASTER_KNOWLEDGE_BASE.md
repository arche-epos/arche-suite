...
**Doc Version: v3.7.9** | *Last updated: September 12, 2026*
...
>
> **Note (v3.7.8):** this token's Issues (R/W) scope is not used — Claude manages Issues via the separate `github-proxy` Worker credential (`github-proxy-2026-07-19`, row above), not this direct-API token. Pages (R/W) is new and untested; see U12 for the historical Pages-admin-via-API failure this might resolve.
>
...
Project Knowledge copies of files Claude cannot fetch/deploy with the dev PAT are **potentially stale**. The dev PAT's write/push scope is `arche-epos/arche-suite` only (current token name/value: see P5 credentials table).
...
**Correction (v3.7.7):** `arche-proxy.js` is no longer fully unreachable for reads. The Cloudflare Developer Platform MCP tool's `workers_get_worker_code` (scriptName: `arche-proxy`) pulls the live deployed Worker source directly — confirmed working Sep 7, 2026, used to diagnose the Lexicon `max_tokens` bug and add per-tool token tracking same session. **Deploy access is unchanged and still fully manual**: this MCP tool has no corresponding write/deploy call, so Boss must still paste any edited Worker source into the Cloudflare dashboard (Workers → arche-proxy → Edit Code → Save & Deploy) — never assume a push happened just because a read did.
...
- For `arche-proxy.js`: pull live source via `workers_get_worker_code` (scriptName: `arche-proxy`) before editing — do not assume the PK/local-backup copy is current. Deploying an edit still requires Boss to paste it into the Cloudflare dashboard; Claude cannot push Worker code.
...
All API calls in the private build go through `https://arche-proxy.archestudytools.workers.dev`. No API keys on the client device. **Not git-tracked; deploy is dashboard-paste-only** (Boss must paste any edit into Cloudflare → Workers → arche-proxy → Edit Code → Save & Deploy). **Read access confirmed Sep 7, 2026** via the Cloudflare Developer Platform MCP tool `workers_get_worker_code` (scriptName: `arche-proxy`) — pulls live deployed source directly, no more need to trust a possibly-stale local backup or PK copy for reads. Still use `workers_get_worker_code` before editing rather than assuming any saved copy is current.
...
**Note on this local copy:** this file is the full, unredacted version (matches Project
Knowledge) — the copy pushed to GitHub has the two P5 credential values redacted, since
git is more exposed than Project Knowledge or this local machine. Use this copy or the PK
copy when an actual credential value is needed; never the GitHub copy.
...
2. **Whenever Claude presents an updated copy of any living-reference doc, resync BOTH
   ...
   the matching local `_Reference/KB/` (or `_Reference/MKB/` for this file) folder via
   Filesystem MCP (already a standing SOP in `Arché-Offline-Backup\_MANIFEST.md`, in place
   since Aug 10 but not consistently followed — this file's local snapshot was still
   v3.7.0/v3.7.4 from Aug 10 when Project Knowledge had already reached v3.7.5). (b) Push
   the corrected file to its repo's `docs/` folder per the pattern above, where one
   exists. Doing both closes the loop immediately instead of relying on a future session
   to notice the drift.
...
### XP-22: Removing an "Unused" Namespace Import Can Break a Window-Bridging Loop
**Discovered:** Pilgrim Private v4.34.15 — Sep 12, 2026 (white screen, zero interactivity, reported immediately by Boss on refresh)
**Pattern:** `app.js` had a namespace import (`import * as Utils from './utils.js'`) that looked unused at its point of use — the only call site referencing it (a stale line writing `Utils.ACTIVE_USER` into a settings field) was being removed in the same edit. The import was deleted as part of that cleanup. But `app.js` also has a window-bridging loop near the bottom of the file (`[Utils, Storage, TTS, Sync, StudyTools, UI].forEach(function(mod){ ... window[name] = mod[name] ... })`) that exposes every module's exports to `window` for `index.html`'s inline `onclick=` handlers — and that loop referenced `Utils` directly, unconditionally, every load. Deleting the import left `Utils` undefined at that line, throwing a `ReferenceError` the instant the module evaluated — before any event listener got wired. The app never crashed loudly; it just silently stopped executing partway through boot, rendering a static, unresponsive shell.
**Root cause:** A namespace import can have two different kinds of "usage" in a file: named property access at specific call sites (grep-visible, easy to audit) and inclusion in a bulk/dispatch construct like an array, object, or loop (still a real reference, but easy to miss on a quick read since the identifier doesn't appear as `Utils.something`). `node --check` only validates syntax, not runtime reference resolution, so a now-undefined identifier used only inside a bulk construct passes syntax-check clean and fails silently at runtime.
**Fix:** Removed only the actually-dead code (the stale override block); restored the `Utils` import since the bridging loop still needs it. Redone in v4.34.17.
**Check in:** Any file with a similar module-namespace bridging/dispatch pattern (arrays of imported namespaces, `Object.keys(mod)` loops, etc.) in Pilgrim Private (`app.js`), and audit for the same shape before removing a namespace import in any app. Prevention rule: before deleting any `import * as X` line, grep the WHOLE file for the bare identifier `X` (not just `X.`) — a bulk construct like `[X, Y, Z].forEach(...)` references the identifier without a dot-access pattern that a narrower search would catch.
