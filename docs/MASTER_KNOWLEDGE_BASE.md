...
**Doc Version: v3.7.7** | *Last updated: September 7, 2026*
...
|---|---|---|
| v3.7.7 | Sep 7, 2026 | **Corrected a standing inaccuracy in U12 and A8**: both previously stated `arche-proxy.js` has no live-read path and is "genuinely unreachable," requiring Boss to paste/confirm current content before any edit. This is no longer true — the Cloudflare Developer Platform MCP tool's `workers_get_worker_code` (scriptName: `arche-proxy`) pulls the live deployed Worker source directly, confirmed working Sep 7, 2026 (used same session to diagnose the Lexicon `max_tokens` truncation bug and add per-tool token/truncation tracking to `/admin/timing`). **Deploy access is unchanged** — this MCP tool has no write/deploy counterpart, so Boss still must paste any edited Worker source into the Cloudflare dashboard; that half of the old constraint stands. |
