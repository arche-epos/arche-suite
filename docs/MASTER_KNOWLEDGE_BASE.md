...
**Doc Version: v3.7.8** | *Last updated: September 7, 2026*
...
|---|---|---|
| v3.7.8 | Sep 7, 2026 | **Manual round-trip test of Diagnostics & Error Log (flagged Aug 30, carried forward unconfirmed across three handoffs) confirmed passing by Boss.** Also corrected P1 staleness caught same session: **Pilgrim Private** row corrected v4.30.3 -> live **v4.30.6** (unified Pilgrim Guide FAB, commit `2d7f7ab`); **pilgrim-admin** row corrected v1.7.0 -> live **v1.8.0** (fileBytes/fileCounts wired into Resource Storage card). New living reference doc `pilgrim-guide-app-help-reference.md` added to `arche-suite/docs/` (commit `7d92a20`) — the reference text the future Pilgrim Guide App Help mode will use to judge whether a question is about the app; must be kept current with any Pilgrim Private nav/tab/feature change (see `ways-of-working.md` in Claude's project memory for this rule). |
...
|---|---|---|---|---|
| Arché Study Tools (Pilgrim Private) | `arche-epos/arche-suite` | `archestudytools.com/pilgrim-private/` | Vanilla JS | **v4.30.6 -- LIVE ON MAIN, confirmed Sep 7, 2026 via direct fetch.** Since v4.30.3 (Sep 4): unified Pilgrim Guide FAB (v4.30.6, commit `2d7f7ab`) -- one floating gold FAB bottom-right on every screen, Library shows New Study/Pilgrim Guide menu, all other tabs open `openPilgrimGuide()` (currently a stub toast; Phase 2 chat UI/App Help/Scripture Finder not yet built, see `spec-pilgrim-assistant-v2.md`). Manual round-trip test of Diagnostics & Error Log confirmed passing by Boss Sep 7, 2026 -- no longer carry-forward. Bridge-check baseline clean. PIN auth + per-user namespacing confirmed fully live. Playwright test scaffolding (`pilgrim-private/tests/`) still NOT started -- remains the declared top-priority workstream. |
| Arché Study Tools (pilgrim-admin) | `arche-epos/arche-suite` | `archestudytools.com/pilgrim-admin/` | Vanilla JS | **v1.8.0 -- LIVE, confirmed Sep 7, 2026.** Boss-only analytics dashboard for Pilgrim Private. Since v1.6.0 (Sep 4): AI Tool Run Timing card raised max_tokens data-gathering (v1.7.0), and `fileBytes`/`fileCounts` wired into the Resource Storage card as an "Uploaded (range)" stat row (v1.8.0). Cards: health/rate-limits, ranked usage, per-tester breakdown, Errors, Diagnostic Runs, AI Tool Run Timing, Resource Storage. No row cap/pagination. |
...
| `github-proxy-worker2.0` (regenerated) | `[REDACTED — see PK or local unredacted copy]` | **Sep 16, 2026** | MKB + used directly via `bash_tool` | **Session/dev token.** Claude's direct `api.github.com` calls during dev work (Git Data API blob/tree/commit/ref flow). Confirmed working July 20, 2026. |
