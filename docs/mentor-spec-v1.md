...
## Bootstrap Security (First-Run Admin)

**Current behavior (POC):** On first load, if no users exist in KV, the app shows a First Run Setup screen — the first person to reach it claims admin. Acceptable for private/trusted deployments where the deployer logs in immediately after launch.

**If used at larger scope, three options (ranked by recommendation):**

### Option 1 — Bootstrap Passphrase (Recommended next step)
Add `MENTOR_SETUP_KEY` secret to the `arche-proxy` Worker env. First-run screen requires it before creating the admin. Ignored after first admin exists. Trivial to implement — one Worker secret, one frontend field.

### Option 2 — Pre-seed Admin at Deploy Time
No first-run screen. Admin account is created via deploy script or manual KV entry on every new instance. Cleaner UX, slightly more ops work per deployment.

### Option 3 — Invite Code Per User (Best for multi-org)
Every account creation (including first admin) requires a one-time code generated by the Worker owner. Scales to multi-church or multi-org deployments where each instance is independently owned. Most build effort.

**Decision logged:** Build Option 1 (passphrase) before any rollout beyond the originating church/org.
