...
export var SK='bsn_studies_v2', SK_SETT='bsn_settings_v1', SK_TAGS='bsn_tags_v1', SK_TAGS_DEL='bsn_tags_deleted_v1';
...
export var SK_STREAK='bsn_streak', SK_TTS_SETT='bsn_tts_sett', SK_WORDS='bsn_words_global';
...
export var studies=[], cur=null, online=navigator.onLine, sett={scrMode:'auto', lastPasteTrans:'', defaultTrans:'esv', diagFeedback:false};
var _diagResults=[];
export var hdrCollapsed=false, scrCollapsed=false, studyScope='passage';
export var activeRefIdx=0;
var _editingTagId=null;
export var _pendingDeleteId=null;
var _pendingDeleteTagId=null;
export var _pendingDeleteRefIdx=null;
 ...
 */
export function closeOverlay(id){document.getElementById(id).classList.remove('on');}
 ...
 */
export function toast(msg){var el=document.getElementById('toast');el.textContent=msg;el.style.background='';el.style.color='';el.style.fontSize='';el.style.borderColor='';el.classList.add('show');clearTimeout(_tt);_tt=setTimeout(function(){el.classList.remove('show');},2400);}
/**
 ...
 */
export function htmlToText(html){if(!html)return '';var d=document.createElement('div');d.innerHTML=html.replace(/<\/(p|div|li|br|h[1-6]|td|th|tr)>/gi,' ');return d.textContent||d.innerText||'';}
/**
 ...
 */
export function setAppHeight(){document.documentElement.style.setProperty('--app-height',window.innerHeight+'px');}
/**
 ...
 */
export function updateOffline(){document.getElementById('offbadge').classList.toggle('show',!online);}
/**
 ...
 */
export function bookOrder(ref){if(!ref)return 999;var r=ref.trim();for(var i=0;i<BIBLE_BOOKS.length;i++){if(r.toLowerCase().startsWith(BIBLE_BOOKS[i].toLowerCase()))return i;}return 999;}
/**
 ...
 */
export function fmtDate(d){if(!d)return '';return new Date(d+'T12:00:00').toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'});}
/**
 ...
 */
export function pdfSafe(str){var s=String(str).replace(/\u2022/g,'-').replace(/\u2013|\u2014/g,'-').replace(/\u201c|\u201d/g,'"').replace(/\u2018|\u2019/g,"'"),out='';for(var i=0;i<s.length;i++){var c=s.charCodeAt(i);
 ...
 */
export function prevDay(d){var dt=new Date(d+'T12:00:00');dt.setDate(dt.getDate()-1);return dt.toISOString().split('T')[0];}
...
export function setStudies(arr){studies=arr;}
/** @param {Object|null} s - New active study (or null) */
export function setCur(s){cur=s;}
/** @param {number} n - New active reference index */
export function setActiveRefIdx(n){activeRefIdx=n;}
/** @param {string} s - New study scope ('passage'|'book') */
export function setStudyScope(s){studyScope=s;}
/** @param {string|null} id - Study ID staged for deletion */
export function setPendingDeleteId(id){_pendingDeleteId=id;}
/** @param {number|null} idx - Ref index staged for deletion */
export function setPendingDeleteRefIdx(idx){_pendingDeleteRefIdx=idx;}
 ...
 */
export function makeRef(type){return {id:'ref_'+Date.now(),reference:'',translation:sett.defaultTrans||'esv',pastedTranslation:'',scriptureText:'',type:type||'primary',deep:{lexical:'',grammar:'',historical:'',cultural:'',crossrefs:'',lexical_book:'',grammar_book:'',historical_book:'',cultural_book:'',crossrefs_book:'',studyScope:'passage'}};}
/**
 ...
 */
export function migrateStudy(s){
 ...
 */
export function activeRef(){if(!cur||!cur.refs||!cur.refs.length)return null;return cur.refs[Math.min(activeRefIdx,cur.refs.length-1)];}
 ...
 */
export function migrateLegacyKey(legacyKey,namespacedKey){
