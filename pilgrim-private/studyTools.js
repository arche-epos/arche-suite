...
function buildPrompt(tool,ref,trans,scope){
  ...
  var prompts={
    ...
    historical:base+intro+' Provide a concise historical overview -- readable for someone without seminary background, do not truncate:\n\nSUMMARY OF THE DATA PRESENTED\n[2-4 sentence plain-language overview: roughly when this was written, by whom, and the historical backdrop]\n\nTIME PERIOD & AUTHOR\n[The traditionally held date range and author, stated plainly in 2-3 sentences — no extended scholarly debate]\n\nPOLITICAL & HISTORICAL LANDSCAPE\n[The key political/historical context in 2-3 sentences]\n\nORIGINAL AUDIENCE\n[Who originally received this, in 1-2 sentences]\n\nComplete all sections in plain, accessible language.'+noC,
...
async function runTool(tool){
  ...
  try{
    ...
    var res=await fetch(WORKER_URL+'/groq',{method:'POST',headers:{'Content-Type':'application/json','X-Tester-Id':ACTIVE_USER||'unknown'},body:JSON.stringify({model:'openai/gpt-oss-120b-Turbo',messages:[{role:'system',content:'Reasoning: low'},{role:'user',content:prompt}],max_tokens:16384,temperature:0.2})});
...
async function expandCurrentTool(){
  ...
  try{
    ...
    if(more){var combined=existing+'\n\n---\n\n'+more;if(!ar.deep)ar.deep={};ar.deep[aiActiveTab]=combined;aiPanelResults[aiActiveTab]=combined;saveStudy(true);renderAIPanelContent(aiActiveTab);toast('Added deeper detail');}
  }catch(e){toast('Go Deeper failed: '+e.message);}
  btn.style.opacity='';btn.style.pointerEvents='';lbl.textContent='Go Deeper \u2014 full scholarly detail';_expandRunning=false;
}
/**
...
async function continueCurrentTool(){
  ...
  try{
    ...
    var res2=await fetch(WORKER_URL+'/groq',{method:'POST',headers:{'Content-Type':'application/json','X-Tester-Id':ACTIVE_USER||'unknown'},body:JSON.stringify({model:'openai/gpt-oss-120b-Turbo',messages:[{role:'system',content:'Reasoning: low'},{role:'user',content:contPrompt}],max_tokens:2048,temperature:0.2})});
...
async function runLexiconLookup(){
  ...
  try{
    var r=await fetch(WORKER_URL+'/groq',{method:'POST',headers:{'Content-Type':'application/json','X-Tester-Id':ACTIVE_USER||'unknown'},body:JSON.stringify({model:'openai/gpt-oss-120b-Turbo',max_tokens:3000,messages:[{role:'system',content:'Reasoning: low'},{role:'user',content:prompt}]})});
    var d=await r.json();
...
async function resRunOCR(id){
  ...
  try{
    ...
    var resp=await fetch(WORKER_URL+'/gemini-ocr',{method:'POST',headers:{'Content-Type':'application/json','X-Tester-Id':ACTIVE_USER||'unknown'},body:JSON.stringify(body)});
